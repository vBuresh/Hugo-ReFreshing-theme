...
Markdown (маркда́ун) — это современный, широко распрострненный язык разметки. Он отличается необычайной постотой, логичностью и легкостью. Элементы форматирование помещаюся непостредственно в текст документа. При этом читаемость текста человеком полностью сохраняется, также сохраняется пригодность текста для машинного преобразования в другие языки и форматы - HTML, Rich Text, .odt, PDF и др.
